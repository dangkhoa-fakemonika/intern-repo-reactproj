...
export const NumberTextField = memo(function NumberTextField(props : NumberTextFieldProps){
  ...
  return (
    <label hidden={props.hidden} className={"relative items-center my-2"}>
      <div className={"absolute duration-300 transition-all z-10 " + ((value) ? "text-sm -my-3 mx-2 bg-white text-palette" : "m-2 opacity-60")}>{props.name}</div>
      <input className={"w-full outline rounded hover:outline-palette hover:shadow duration-500 focus:outline-palette px-2 py-2 text-lg relative"}
